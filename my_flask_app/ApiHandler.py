...
def getMostPopularVideosByCountry(pRegionCode, pNumOfRecords):
    ...
    for video in response.get('items', []):
        ...
        responseData.append([chartCountry, remove_emojis(title), remove_emojis(description), publishedAt, channelid, channeltitle, category_name, duration, definition, projection, views, likeCount, commentCount, extractionDate])
    
    # Erstelle einen DataFrame aus der Liste    
    df = pd.DataFrame(responseData, columns=['Country_Chart', 'Title', 'Description', 'Published_At', 'Channel_ID', 'Channel_Title', 'Category_Name', 'Duration', 'Definition', 'Projection', 'Views', 'Like_Count', 'Comment_Count', 'Extraction_Date'])
    
    # Konvertierung von 'Published_At' und 'Extraction_Date' in datetime64
    df['Published_At'] = pd.to_datetime(df['Published_At']).dt.tz_localize(None)

    print(df["Published_At"][0])
    ...
    df = df.astype({
        ...
        'Duration': 'int',  # oder 'int', abhängig davon, ob Sie Sekunden als ganze Zahlen speichern möchten
